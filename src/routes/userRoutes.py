...
@router.post("/create") # to create something new use POST API call
async def create_user(clerk_webhook_data: dict):
    ...
    2. Check the event type - our case type - "user.created"
    3. Extract and validate the user data
    4. Extract and validate the clerk_id
    5. Check if the user already exists to prevent duplicate

...
def partition_document(temp_file: str, file_type: str, source_type: str = "url"):
    ...
    dispatch = { # initalizing a dict with lambda to make a controlled execution
        "pdf": lambda: partition_pdf(
            filename = temp_file, # specify the filename path
            strategy= "hi_res", # Most accutare but slower processing method of extraction
            infer_table_structure=True, # keep tables as structured HTML, not jumbled text
            extract_image_block_types=["Image"], # Grab images found in pdf.
            extract_image_block_to_payload=True # Store image as base64 strings in the payload
        ),
    ...
    return dispatch[file_type]()

def analyze_elements(elements):
    """analzye the elements and return the summary"""
    ...
    for element in elements:
        element_name = type(element).__name__ # __name__ is a special attribute that returns the class name like "Table" or "NarrativeText"
...
def get_page_number(chunk, chunk_index):
    ...
    # fallback: usr chunk index as page number
    return chunk_index
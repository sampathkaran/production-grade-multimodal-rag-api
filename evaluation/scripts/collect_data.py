...
TEST_QUESTIONS = [
    #"What is the architecture of the described neural network?"
    #"What steps can be taken to nurture wildlife?",
...
def collect_rag_data(project_id: str, questions: list) -> list:
    """Run questions through the RAG pipeline and collect data"""
    ...
    for question in questions:
        ...
        # Retrieve Context
        texts, images, tables, citations = retrieve_context(project_id= project_id, user_query=question)
        
        # Prepare contexts for RAGAS
        contexts = texts + [f"[TABLE]\n{table}" for table in tables]

        # Generate answer
        answer = prepare_prompt_and_invoke_llm(user_query=question, texts=texts, images=[], tables=tables)
